...
# 4.3 创建GridSearchCV对象 → 寻找最有超参，使用网格搜索 + 交叉验证
# 参1：要计算最优超参的模型对象
# 参2： 该模型可能出现的值
# 参3：交叉验证的折数，这里的4折表示：每个超参组合，都会进行4次交叉验证 这里共计是4*10=40次。
# 返回值： estimator → 处理后的模型对象
estimator = GridSearchCV(estimator, param_dict, cv=4)
# 4.4 具体的模型训练
estimator.fit(x_train, y_train)
# 4.5 打印最优超参组合
print(f'最优评分：{estimator.best_score_}')  # 最优评分：0.9666666666666668
print(f'最有超参组合：{estimator.best_params_}')  # 最有超参组合：{'n_neighbors': 3}
print(f'最优的估计器对象：{estimator.best_estimator_}')  # 最优的估计器对象：KNeighborsClassifier(n_neighbors=3)
...
# 5.4 模型评估
# 参1：预测集 参2：预测集
print(f'准确率：{accuracy_score(y_test, y_pre)}')  # 准确率：0.9666666666666667
